...
for da in das:
    ...
    mysum=0
    mydiff=[]
    flogout = open(f'{da}I2M_bmd_out.txt' , 'w')
    # 添加第3列
    failed=[]
    new_column = []
    rdkit_canconlized=[]
    ...
    csv_path =f"/home/jovyan/volume/samba_share/from_docker/data/work_space/ori/real/{da}.csv" #279
    acs_dir = f"/home/jovyan/volume/samba_share/from_docker/data/work_space/ori/real/{da}" #*smi 279、
    ...
    for i, row in df.iterrows():
        ...
        # smiles = df.loc[i,1]
        png_filename = df.file_path[i].split('/')[-1]
        smiles = df.SMILES[i]
        ...
        try:
            ...
            print(f'I2M output problems @@ {img_path} ')
            print(e)
            # if smiles_pred:
            # smiles_pred=None
    ...
    flogout.write(f'I2M_bmd@@{da}:: match--{mysum},unmatch--{len(mydiff)},failed--{len(failed)},correct %{100*mysum/len(df)} ')
    flogout.close()
    # if da=='hand':
    ...
    rdkit_canconlized_df.to_csv(f"{da}_bmd_output.csv", index=False)
    rdkit_canconlized_df_diff.to_csv(f"{da}_bmd_diff_output.csv", index=False)
...
nohup python I2Mbmd_test.py > all_rI2Mbmd.out 2>&1 & 
