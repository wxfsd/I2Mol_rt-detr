...
#AS new albumentations changed
from albucore.utils import maybe_process_in_chunks
import cv2
import math
import random
import numpy as np
...
def safe_rotate_enlarged_img_size(angle: float, rows: int, cols: int):
    """safe_rotate_enlarged_img_size had been removed from albumentations.augmentations.geometric.functional
    """
    deg_angle = abs(angle)
    # The rotation angle
...
def safe_rotate(
    ...
    warp_affine_fn = maybe_process_in_chunks(
        cv2.warpAffine,
        M=rotation_mat,
