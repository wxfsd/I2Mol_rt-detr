...
def _verify_chirality(mol, coords, symbols, edges, debug=False):
    try:
        ...
        for i, (x, y) in enumerate(coords):
            conf.SetAtomPosition(i, (x, 1 - y, 0))#2D 
...
def get_smiles_from_symbol(symbol, mol, atom, bonds):
    ...
    if len(symbol) > 20:
        print(symbol,'may not be chemistry!!!!!@get_smiles_from_symbol')
        return None
...
def _convert_graph_to_smiles(coords, symbols, edges, image=None, debug=False):
    ...
    try:
        ...
        if image is not None:
            ...
            ratio = width / height
            coords = [[x * ratio * 10, y * 10] for x, y in coords]# TODO just as judger of chem-image or not with coords
