...
def draw_text(draw,
    ...
    # display_str_heights = [font.getsize(ds)[1] for ds in display_str]#error NOTE NEED Pillow<=9.5.0
    ...
    for ds in display_str:
        # print(font.getbbox(ds),"@left, top, left + width, top + height")
        a1,a2,a3,a4= font.getbbox(ds)
        text_width, text_height = a3-a1,a4-a2
...
def draw_objs(image: Image,
    ...
    colors = [ImageColor.getrgb(STANDARD_COLORS[cls_ % len(STANDARD_COLORS)]) for cls_ in classes]

    if draw_boxes_on_image:
        # Draw all boxes onto image.
        draw = ImageDraw.Draw(image)
        for box, cls_, score, color in zip(boxes, classes, scores, colors):
            left, top, right, bottom = box
            #TODO get the * box cut and string OCR recong
            # 绘制目标边界框
            draw.line([(left, top), (left, bottom), (right, bottom),
                       (right, top), (left, top)], width=line_thickness, fill=color)
            # 绘制类别和概率信息
            draw_text(draw, box.tolist(), int(cls_), float(score), category_index, color, font, font_size)
